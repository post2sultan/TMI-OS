...
$job=Invoke-RestMethod "$BaseUrl/api/publishing/instagram/next" -Headers $localHeaders
...
function Report([string]$path,[hashtable]$body){Invoke-RestMethod -Method Post "$BaseUrl/api/publishing/instagram/$($job.job_id)/$path" -Headers $localHeaders -ContentType "application/json" -Body ($body|ConvertTo-Json -Compress)|Out-Null}
try {
  ...
  $container=Invoke-RestMethod -Method Post "https://graph.instagram.com/v24.0/$account/media" -Body @{media_type="REELS";video_url="$public/$fileName";caption=[string]$job.caption;share_to_feed="true";access_token=$token}
  ...
  if(-not $published.id){throw "Instagram did not return a media ID."}; Report "complete" @{media_id=[string]$published.id}; Write-Host "Public Instagram Reel publishing PASSED for campaign $($job.campaign_id)."
