...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/models/campaign_document.py','/app/app/repositories/campaign_document_repository.py','/app/alembic/versions/b8e1f2a3c4d5_add_campaign_source_documents.py','/app/tests/test_campaign_document_repository.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Campaign-document migration failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P1-01 test suite failed."
    }

    Write-Host ""
    Write-Host "P1-01 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P1-01 installation FAILED. Rolling back..." -ForegroundColor Red
    try {
        docker compose -f $ComposeFile exec -T backend alembic downgrade 9c7b2d4e6f80 | Out-Null
    }
    catch {
    }
