...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/main.py','/app/app/services/analysis/pipeline.py','/app/app/services/analysis/quality_validator.py','/app/tests/test_analysis_quality_validator.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    Write-Host ""
    Write-Host "P0-01 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P0-01 installation FAILED. Rolling back..." -ForegroundColor Red
    Restore-Backup
