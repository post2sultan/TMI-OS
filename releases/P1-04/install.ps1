...
$Release = Join-Path $Root "releases\P1-04"
...
$Targets = @(
    ...
    "backend\app\services\evidence_confidence.py",
    "backend\alembic\versions\e1b4c6d8f0a2_add_evidence_confidence.py",
    "backend\tests\test_campaign_document_repository.py",
    "backend\tests\test_evidence_confidence.py",
    "docs\BACKLOG.md"
)
...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/models/campaign_document.py','/app/app/repositories/campaign_document_repository.py','/app/app/services/evidence_confidence.py','/app/alembic/versions/e1b4c6d8f0a2_add_evidence_confidence.py','/app/tests/test_campaign_document_repository.py','/app/tests/test_evidence_confidence.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Evidence-confidence migration failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P1-04 test suite failed."
    }

    Write-Host ""
    Write-Host "P1-04 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P1-04 installation FAILED. Rolling back..." -ForegroundColor Red
    try {
        docker compose -f $ComposeFile exec -T backend alembic downgrade d0a3b5c7e9f1 | Out-Null
    }
    catch {
    }
