...
if ($LASTEXITCODE -ne 0) { throw "P1-04 test suite failed." }
...
docker compose -f $ComposeFile exec -T backend python -B -c "from datetime import datetime,timezone; from app.core.database import SessionLocal; from app.models.campaign_document import CampaignDocument; from app.repositories.campaign_document_repository import campaign_document_repository as repo; db=SessionLocal(); now=datetime.now(timezone.utc); item=repo.create(session=db,campaign_id=$CampaignId,document_type='news_article',source_url='https://example.com/validation',title='validation',extracted_text=' '.join(['evidence']*60)); repo.apply_extraction(session=db,document=item,title='validation',extracted_text=item.extracted_text,published_at=now,retrieved_at=now,language='en',brand='TMI',media_assets=[],extraction_status='SUCCESS',extraction_method='validation'); item_id=item.id; assert item.confidence_score==0.95 and item.evidence_origin=='web_extraction' and item.evidence_representation=='direct_quote' and item.supporting_source=='https://example.com/validation'; db.rollback(); assert db.get(CampaignDocument,item_id) is None; db.close(); print('CONFIDENCE_ROUND_TRIP_OK')"
if ($LASTEXITCODE -ne 0) { throw "Confidence rollback round trip failed." }
...
if (($Migration | Out-String) -notmatch "e1b4c6d8f0a2") {
    throw "P1-04 migration is not active."
}

Write-Host ""
Write-Host "P1-04 validation PASSED."
...
Write-Host "Backend tests passed:       27"
