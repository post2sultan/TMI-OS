...
if (-not (Test-Path $LatestBackupFile)) {
    throw "No P1-04 backup reference exists."
}

$Backup = (Get-Content $LatestBackupFile -Raw).Trim()
if (-not $Backup -or -not (Test-Path $Backup)) {
    throw "P1-04 backup folder not found: $Backup"
}

docker compose -f $ComposeFile exec -T backend alembic downgrade d0a3b5c7e9f1
if ($LASTEXITCODE -ne 0) {
    throw "Evidence-confidence migration rollback failed."
}
...
for ($Attempt = 1; $Attempt -le 40; $Attempt++) {
    try {
        ...
        if ($Health.status -eq "healthy") {
            Write-Host "P1-04 rollback PASSED."
