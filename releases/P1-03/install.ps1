...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/api/extraction.py','/app/app/models/campaign_document.py','/app/app/repositories/campaign_document_repository.py','/app/app/services/evidence_deduplicator.py','/app/alembic/versions/d0a3b5c7e9f1_add_evidence_deduplication.py','/app/tests/test_campaign_document_repository.py','/app/tests/test_evidence_deduplicator.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Evidence-deduplication migration failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P1-03 test suite failed."
    }

    Write-Host ""
    Write-Host "P1-03 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P1-03 installation FAILED. Rolling back..." -ForegroundColor Red
    try {
        docker compose -f $ComposeFile exec -T backend alembic downgrade c9f2a4b6d8e0 | Out-Null
    }
    catch {
    }
