...
$CleanRoot = Join-Path $Root "tmp\p3-mvp-01-$Stamp"
...
$BackendBackup = "tmi-os-backend:pre-p3-mvp-01-$Stamp"
$FrontendBackup = "tmi-os-frontend:pre-p3-mvp-01-$Stamp"
...
$DbBackupName = "p3-mvp-01-$Stamp.sql"
...
try {
    ...
    Write-Host "P3-MVP-01 installation PASSED."
}
catch {
    docker compose --env-file $EnvFile --project-name tmi-production `
        -f $ComposeFile run --rm backend alembic downgrade d6a9c2e4f7b1 | Out-Null
