...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/core/settings.py','/app/app/services/vector/qdrant_service.py','/app/app/services/prompt_service.py','/app/tests/test_production_configuration.py','/app/tests/test_prompt_packaging.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "GL-02 test suite failed."
    }

    & (Join-Path $Root "scripts\validate-production-container.ps1") -Root $Root

    Write-Host ""
    Write-Host "GL-02 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "GL-02 installation FAILED. Rolling back..." -ForegroundColor Red
    Restore-Backup
    try {
