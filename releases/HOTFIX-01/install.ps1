...
$CleanRoot = Join-Path $Root "tmp\hotfix-01-$Stamp"
...
$BackupImage = "tmi-os-backend:pre-hotfix-01-$Stamp"
...
try {
    ...
    if ($LASTEXITCODE -ne 0) { throw "HOTFIX-01 backend build failed." }

    docker compose --env-file $EnvFile --project-name tmi-production `
        -f $ComposeFile up -d --no-deps --wait backend
    if ($LASTEXITCODE -ne 0) { throw "HOTFIX-01 deployment failed." }

    & (Join-Path $Release "validate.ps1") -Root $Root
    Write-Host "HOTFIX-01 installation PASSED."
}
catch {
