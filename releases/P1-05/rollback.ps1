...
if (-not (Test-Path $LatestBackupFile)) {
    throw "No P1-05 backup reference exists."
}

$Backup = (Get-Content $LatestBackupFile -Raw).Trim()
if (-not $Backup -or -not (Test-Path $Backup)) {
    throw "P1-05 backup folder not found: $Backup"
}

docker compose -f $ComposeFile exec -T backend alembic downgrade e1b4c6d8f0a2
if ($LASTEXITCODE -ne 0) {
    throw "Campaign-lifecycle migration rollback failed."
}
...
for ($Attempt = 1; $Attempt -le 40; $Attempt++) {
    try {
        ...
        if ($Health.status -eq "healthy") {
            Write-Host "P1-05 rollback PASSED."
