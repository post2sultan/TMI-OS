...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/services/analysis/pipeline.py','/app/tests/test_analysis_logging.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    Write-Host ""
    Write-Host "P0-04 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P0-04 installation FAILED. Rolling back..." -ForegroundColor Red
    Restore-Backup
    try {
