...
$Release = Join-Path $Root "releases\P0-02"
...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/services/ai/router.py','/app/app/services/analysis/pipeline.py','/app/tests/test_analysis_retry.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P0-02 test suite failed."
    }

    Write-Host ""
    Write-Host "P0-02 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P0-02 installation FAILED. Rolling back..." -ForegroundColor Red
    Restore-Backup
