...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/main.py','/app/app/models/analysis.py','/app/app/services/analysis/pipeline.py','/app/app/services/analysis/repository.py','/app/app/services/analysis/review_service.py','/app/alembic/versions/a3d6f8b0c2e4_add_analysis_versioning.py','/app/tests/test_analysis_versioning.py','/app/tests/test_campaign_review_api.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Analysis-versioning migration failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P1-07 test suite failed."
    }

    Write-Host ""
    Write-Host "P1-07 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P1-07 installation FAILED. Rolling back..." -ForegroundColor Red
    try {
        docker compose -f $ComposeFile exec -T backend alembic downgrade f2c5d7e9a1b3 | Out-Null
    }
    catch {
    }
