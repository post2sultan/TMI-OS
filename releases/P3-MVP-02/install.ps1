...
$BackendBackup = "tmi-os-backend:pre-p3-mvp-02-$Stamp"
$FrontendBackup = "tmi-os-frontend:pre-p3-mvp-02-$Stamp"
...
$DbBackupName = "p3-mvp-02-$Stamp.sql"
...
try {
    ...
    Write-Host "P3-MVP-02 installation PASSED."
}
catch {
    docker compose --env-file $EnvFile --project-name tmi-production `
        -f $ComposeFile run --rm backend alembic downgrade e7b1d3f5a9c2 | Out-Null
