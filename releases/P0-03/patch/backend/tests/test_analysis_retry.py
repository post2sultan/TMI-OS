...
class AnalysisRetryTests(unittest.TestCase):
    ...
    def test_invalid_json_retries_once_with_correction(self) -> None:
        ...
        self.assertEqual(
            pipeline.run_repository.record_attempt.call_count,
            2,
        )
        first_record = (
        ...
        self.assertEqual(
            first_record["validation_status"],
            "failed",
        )
        self.assertEqual(
            second_record["validation_status"],
            "passed",
        )
        self.assertEqual(
