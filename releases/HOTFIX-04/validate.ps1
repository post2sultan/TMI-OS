...
$sidebar = Get-Content -LiteralPath (Join-Path $Root "frontend\src\components\layout\Sidebar.tsx") -Raw
...
if (-not $sidebar.Contains('isActive ? "text-slate-950"') -or -not $sidebar.Contains('isActive ? "text-slate-600"')) {
    throw "Active navigation contrast guard is missing."
}
if (-not $SourceOnly) {
