...
if ($LASTEXITCODE -ne 0) { throw "P1-02 test suite failed." }
...
docker compose -f $ComposeFile exec -T backend python -B -c "from datetime import datetime,timezone; from app.core.database import SessionLocal; from app.models.campaign_document import CampaignDocument; from app.repositories.campaign_document_repository import campaign_document_repository as repo; db=SessionLocal(); item=repo.create(session=db,campaign_id=$CampaignId,document_type='manual_observation',title='validation',extracted_text='temporary'); repo.apply_extraction(session=db,document=item,title='validation',extracted_text='structured',published_at=None,retrieved_at=datetime.now(timezone.utc),language='en',brand='TMI',media_assets=['validation.jpg'],extraction_status='SUCCESS',extraction_method='validation'); item_id=item.id; assert item.extraction_status=='success' and item.media_assets==['validation.jpg']; db.rollback(); assert db.get(CampaignDocument,item_id) is None; db.close(); print('EVIDENCE_ROUND_TRIP_OK')"
if ($LASTEXITCODE -ne 0) { throw "Evidence rollback round trip failed." }
...
if (($Migration | Out-String) -notmatch "c9f2a4b6d8e0") {
    throw "P1-02 migration is not active."
}

Write-Host ""
Write-Host "P1-02 validation PASSED."
...
Write-Host "Backend tests passed:       21"
