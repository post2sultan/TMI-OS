...
try {
    ...
    docker compose -f $ComposeFile exec -T backend python -B -c "import ast,pathlib; files=['/app/app/api/extraction.py','/app/app/models/campaign_document.py','/app/app/repositories/campaign_document_repository.py','/app/alembic/versions/c9f2a4b6d8e0_add_structured_evidence_extraction.py','/app/tests/test_campaign_document_repository.py','/app/tests/test_campaign_evidence_extraction.py']; [ast.parse(pathlib.Path(f).read_text(encoding='utf-8-sig'), filename=f) for f in files]; print('SYNTAX_OK')"
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "Evidence-extraction migration failed."
    }
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "P1-02 test suite failed."
    }

    Write-Host ""
    Write-Host "P1-02 installation PASSED."
    Write-Host "Backup: $Backup"
}
catch {
    Write-Host ""
    Write-Host "P1-02 installation FAILED. Rolling back..." -ForegroundColor Red
    try {
        docker compose -f $ComposeFile exec -T backend alembic downgrade b8e1f2a3c4d5 | Out-Null
    }
    catch {
    }
